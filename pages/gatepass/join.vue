<template>
  <v-container>
    <h1 class="text-2xl pb-3">Request Gatepass JOIN INSPECTION</h1>
        ...
        <v-data-table :headers="headers" :items="filteredItems" class="elevation-1" :search="search">
          <template v-slot:item="{ item }">
            <tr @click="handleRowClick(item)">
              <td>{{ item.id }}</td>
              <td>{{ item.DOKUMEN_JOIN_INSPECTION }}</td>
              <td>{{ item.DOKUMEN_KARANTINA }}</td>
              <td>{{ item.DOKUMEN_BEA_CUKAI }}</td>
              <td>{{ item.NO_BL }}</td>
...
const headers = [
  { title: 'ID', value: 'id' },
  { title: 'DOKUMEN JOIN INSPECTION', value: 'DOKUMEN_JOIN_INSPECTION' },
  { title: 'DOKUMEN KARANTINA', value: 'DOKUMEN_KARANTINA' },
  { title: 'DOKUMEN BEA CUKAI', value: 'DOKUMEN_BEA_CUKAI' },
  { title: 'NO BL', value: 'NO_BL' },
  { title: 'STATUS REQUEST', value: 'STATUS_REQUEST' },
...
const fetchdata = async () => {
  ...
  try {
    ...
    const dataFetch = await $fetch(`${apiBase}/gatepass_join_index`, {
      headers: {
...
const fetchDocumentDetails = async (id) => {
  ...
  const token = localStorage.getItem('auth_token');
  const { data, error } = await useFetch(`${apiBase}/gatepass_join_detail `, {
    headers: {
      'Authorization': `Bearer ${token}`
    },
    method: 'POST',
...
const handleSubmit = async () => {
  ...
  const token = localStorage.getItem('auth_token');
  try {
    const responseData = await $fetch(`${apiBase}/gatepass_join_search`, {
      method: 'POST',
      headers: {
